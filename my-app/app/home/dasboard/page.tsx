...
export default function Dashboard() {
  ...
  const fetchJadwalMendatang = async (userId: number) => {
    try {
      const response = await fetch(`/api/jadwal?siswa_id=${userId}&status=dijadwalkan`);
      const data = await response.json();
      
      if (data.success && data.data) {
        // Filter hanya jadwal yang akan datang
        const upcoming = data.data.filter((jadwal: JadwalKonselingWithRelations) => {
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl shadow-lg p-6 text-white">
              ...
              <p className="text-3xl font-bold">{loading ? '...' : dashboardData?.total_konseling || 0}</p>
            ...
            <div className="bg-gradient-to-br from-purple-500 to-purple-600 rounded-xl shadow-lg p-6 text-white">
              ...
              <p className="text-3xl font-bold">{loading ? '...' : dashboardData?.konseling_selesai || 0}</p>
            ...
            <div className="bg-gradient-to-br from-orange-500 to-orange-600 rounded-xl shadow-lg p-6 text-white">
              ...
              <p className="text-3xl font-bold">{loading ? '...' : dashboardData?.konseling_berlangsung || 0}</p>
              <p className="text-xs opacity-75 mt-1">Konseling aktif</p>
            </div>
          </div>
                    ...
                    <div key={jadwal.id} className="border border-gray-700 rounded-lg p-4 hover:bg-gray-800 transition-colors">
                      <div className="flex items-start justify-between">
                        ...
                        <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
                          jadwal.status === 'dijadwalkan' ? 'bg-blue-100 text-blue-700' :
                          jadwal.status === 'berlangsung' ? 'bg-green-100 text-green-700' :
